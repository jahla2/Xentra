...
type ActionRepository interface{Save(context.Context,domain.ActionRequest)error;Get(context.Context,string)(domain.ActionRequest,error)}
type AuditRepository interface{Append(context.Context,domain.AuditEvent)error;List(context.Context)([]domain.AuditEvent,error)}
type RemediationClient interface{ExecuteAction(context.Context,domain.Environment,string,string)(string,error);VerifyAction(context.Context,domain.Environment,string,string)(domain.VerificationResult,error)}
...
func(s *ActionService)Propose(ctx context.Context,incidentID,environmentID,action,target,reason string)(domain.ActionRequest,error){
	if environmentID==""||action==""||target==""||reason==""{return domain.ActionRequest{},errors.New("environmentId, action, target and reason are required")}
...
func(s *ActionService)Approve(ctx context.Context,id,approvedBy string)(domain.ActionRequest,error){
	if approvedBy==""{return domain.ActionRequest{},errors.New("approvedBy is required")}
	item,err:=s.actions.Get(ctx,id);if err!=nil{return domain.ActionRequest{},err}
	if item.Status!="pending_approval"{return domain.ActionRequest{},errors.New("action is not pending approval")}
	env,err:=s.environments.Get(ctx,item.EnvironmentID);if err!=nil{return domain.ActionRequest{},err}
	item.Status="approved";item.ApprovedBy=approvedBy
	_ = s.actions.Save(ctx,item)
	_ = s.appendAudit(ctx,item.EnvironmentID,approvedBy,"action_approved",item.Action+" "+item.Target,true)
...
func(r *MemoryActionRepository)Save(_ context.Context,item domain.ActionRequest)error{r.mu.Lock();defer r.mu.Unlock();r.data[item.ID]=item;return nil}
func(r *MemoryActionRepository)Get(_ context.Context,id string)(domain.ActionRequest,error){r.mu.RLock();defer r.mu.RUnlock();item,ok:=r.data[id];if !ok{return domain.ActionRequest{},errors.New("action not found")};return item,nil}

type MemoryAuditRepository struct{mu sync.RWMutex;items []domain.AuditEvent}
func NewMemoryAuditRepository()*MemoryAuditRepository{return &MemoryAuditRepository{}}
func(r *MemoryAuditRepository)Append(_ context.Context,item domain.AuditEvent)error{r.mu.Lock();defer r.mu.Unlock();r.items=append(r.items,item);return nil}
func(r *MemoryAuditRepository)List(_ context.Context)([]domain.AuditEvent,error){r.mu.RLock();defer r.mu.RUnlock();items:=append([]domain.AuditEvent(nil),r.items...);return items,nil}
