...
func(r *MemoryIncidentRepository)Get(_ context.Context,id string)(domain.Incident,error){r.mu.RLock();defer r.mu.RUnlock();item,ok:=r.data[id];if !ok{return domain.Incident{},errors.New("incident not found")};return item,nil}
func(r *MemoryIncidentRepository)List(_ context.Context)([]domain.Incident,error){r.mu.RLock();defer r.mu.RUnlock();items:=make([]domain.Incident,0,len(r.data));for _,item:=range r.data{items=append(items,item)};return items,nil}
