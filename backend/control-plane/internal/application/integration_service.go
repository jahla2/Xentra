package application
import("context";"errors";"fmt";"sync";"sync/atomic";"github.com/jahla2/Xentra/backend/control-plane/internal/domain")
...
type IntegrationService struct{repo RepositoryIntegrationRepository;credentials GitHubTokenWriter;counter atomic.Uint64}
...
func(s *IntegrationService)ConnectGitHub(ctx context.Context,environmentID,owner,repoName,token string)(domain.RepositoryIntegration,error){if environmentID==""||owner==""||repoName==""||token==""{return domain.RepositoryIntegration{},errors.New("environmentId, owner, repo and accessToken are required")};credentialID,err:=s.credentials.StoreGitHubToken(ctx,token);if err!=nil{return domain.RepositoryIntegration{},err};integration:=domain.RepositoryIntegration{ID:fmt.Sprintf("int-%d",s.counter.Add(1)),EnvironmentID:environmentID,Provider:"github",Owner:owner,Repo:repoName,CredentialID:credentialID};if err:=s.repo.Save(ctx,integration);err!=nil{return domain.RepositoryIntegration{},err};return integration,nil}
