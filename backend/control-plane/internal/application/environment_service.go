...
type EnvironmentService struct{repo EnvironmentRepository;discovery DiscoveryClient;credentials SSHCredentialWriter}
...
func(s *EnvironmentService)Create(ctx context.Context,input CreateEnvironmentInput)(domain.Environment,error){
	...
	id,err:=newResourceID("env");if err!=nil{return domain.Environment{},fmt.Errorf("create environment id: %w",err)}
	connectionType:=input.ConnectionType;if connectionType==""{connectionType="runner"}
	env:=domain.Environment{ID:id,Name:input.Name,Type:input.Type,ConnectionType:connectionType,RunnerURL:input.RunnerURL,SSHHost:input.SSHHost,SSHPort:input.SSHPort,SSHUser:input.SSHUser,SSHHostKeyFingerprint:input.SSHHostKeyFingerprint}
	if env.Type==""{env.Type="development"}
	switch connectionType{
	case"runner":
		if input.RunnerURL==""{return domain.Environment{},errors.New("runnerUrl is required for runner connections")}
	case"ssh":
		...
		if env.SSHPort==0{env.SSHPort=22}
		if s.credentials==nil{return domain.Environment{},errors.New("credential store is unavailable")}
		credentialID,storeErr:=s.credentials.StoreSSH(ctx,domain.SSHCredential{PrivateKey:input.SSHPrivateKey,Passphrase:input.SSHPassphrase});if storeErr!=nil{return domain.Environment{},fmt.Errorf("store ssh credential: %w",storeErr)}
	...
	if err:=s.repo.Save(ctx,env);err!=nil{return domain.Environment{},err}
	return env,nil
}
