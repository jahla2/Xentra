...
func (r *PostgresEnvironmentRepository) Get(ctx context.Context, organizationID, id string) (domain.Environment, error) {
	query := "SELECT id,organization_id,COALESCE(project_id,''),name,environment_type,connection_type,COALESCE(runner_url,''),COALESCE(ssh_host,''),ssh_port,COALESCE(ssh_user,''),COALESCE(ssh_host_key_fingerprint,''),COALESCE(credential_id,''),os,hostname,capabilities FROM environments WHERE organization_id=$1 AND id=$2"
...
func (r *PostgresEnvironmentRepository) List(ctx context.Context, organizationID string) ([]domain.Environment, error) {
	query := "SELECT id,organization_id,COALESCE(project_id,''),name,environment_type,connection_type,COALESCE(runner_url,''),COALESCE(ssh_host,''),ssh_port,COALESCE(ssh_user,''),COALESCE(ssh_host_key_fingerprint,''),COALESCE(credential_id,''),os,hostname,capabilities FROM environments WHERE organization_id=$1 ORDER BY name"
