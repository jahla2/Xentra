...
func (r *PostgresIntegrationRepository) Save(ctx context.Context, item domain.RepositoryIntegration) error {
	query := "INSERT INTO repository_integrations(id,organization_id,environment_id,provider,owner_name,repo_name,credential_id) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(environment_id) DO UPDATE SET organization_id=EXCLUDED.organization_id,provider=EXCLUDED.provider,owner_name=EXCLUDED.owner_name,repo_name=EXCLUDED.repo_name,credential_id=EXCLUDED.credential_id"
	_, err := r.db.ExecContext(ctx, query, item.ID, item.OrganizationID, item.EnvironmentID, item.Provider, item.Owner, item.Repo, item.CredentialID)
	return err
}
...
func (r *PostgresIntegrationRepository) FindByEnvironment(ctx context.Context, organizationID, id string) (domain.RepositoryIntegration, error) {
	...
	query := "SELECT id,organization_id,environment_id,provider,owner_name,repo_name,credential_id FROM repository_integrations WHERE organization_id=$1 AND environment_id=$2"
	err := r.db.QueryRowContext(ctx, query, organizationID, id).Scan(&item.ID, &item.OrganizationID, &item.EnvironmentID, &item.Provider, &item.Owner, &item.Repo, &item.CredentialID)
